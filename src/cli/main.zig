...
pub fn main(init: std.process.Init) !void {
    ...
    if (args.len == 3 and std.mem.eql(u8, args[2], "help")) {
        if (!commands.printCommandHelp(cmd)) {
    ...
    if (std.mem.eql(u8, cmd, "init")) return try commands.handleInitConfig(allocator, config_path);
    ...
    if (std.mem.eql(u8, cmd, "discover")) return try commands.handleFindTartarus(allocator);
